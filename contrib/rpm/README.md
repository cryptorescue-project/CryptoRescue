...
The RPM spec file provided here is for CryptoRescue-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cryptorescue.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[CryptoRescue git repository](https://github.com/cryptorescue-project/CryptoRescue) but are not part of
...
Currently only `Source10` is used, for the example `cryptorescue.conf` file.
...
in the cryptorescue git repository.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cryptorescue
...
needed when 0.12.1 is released, a proper fix is already in the CryptoRescue git
master branch.
...
idea to build CryptoRescue Core against a static version of that library compiled
...
The `cryptorescue.png` file is from the source listed as `Source100`. It is used as
the source for the PNG and XPM files. The generated PNG and XPM files are given
the same timestamp as the source PNG file as a means of indicating they are
derived from it.

## Systemd

This spec file assumes the target distribution uses systemd. That really only
matters for the `cryptorescue-server` package. At this point, most RPM based
...
The `cryptorescue-server` package should have SELinux support. How to properly do
...
CryptoRescue-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cryptorescue/). He has been
packaging CryptoRescue for Fedora at least since 2012.
...
4. I renamed the `cryptorescue` package that contains the Qt GUI to `cryptorescue-project` as
that appears to be how the general population refers to it, in contrast to
`cryptorescue-xt` or `cryptorescue-classic`. I wanted to make sure the general population
