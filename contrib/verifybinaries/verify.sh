...
###   cryptorescuecore.org and cryptorescue.org and compares them.
...
WORKINGDIR="/tmp/cryptorescue_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://cryptorescue.org"
HOST2="https://cryptorescue.org"
BASEDIR="/bin/"
VERSIONPREFIX="cryptorescue-project-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cryptorescue-',
...
if [ $? -ne 0 ]; then
   echo "cryptorescue.org failed to provide signature file, but cryptorescuecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "cryptorescue.org and cryptorescuecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the CryptoRescue Core binary release signing key installed?"
   fi
