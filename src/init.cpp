 ...
 * between cryptorescued, and cryptorescue-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("cryptorescue-shutoff");
    ...
    FlushWallets();
#endif
    GenerateCryptoRescues(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), CRYPTORESCUE_CONF_FILENAME));
    if (mode == HMM_CRYPTORESCUED)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CRYPTORESCUE_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cryptorescue-project/CryptoRescue>";
    const std::string URL_WEBSITE = "<https://cryptorescue.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("cryptorescue-loadblk");
...
/** Sanity checks
 *  Ensure that CryptoRescue is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CryptoRescue version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single CryptoRescue process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CRYPTORESCUE_CONF_FILENAME)).string());
    ...
    GenerateCryptoRescues(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
