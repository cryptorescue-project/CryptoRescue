    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return CryptoRescueUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, CryptoRescueUnits::separatorNever);
            else
                return CryptoRescueUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+CryptoRescueUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
